...
class ArtifactControllerTest {
    ...
    void findById() throws Exception {
        when(artifactService.findById("1250808601736515584")).thenReturn(list.get(0));
        when(artifactService.findById("1250808601744904193")).thenReturn(list.get(1));
        mockMvc.perform(MockMvcRequestBuilders.get("/artifacts/1250808601736515584").contentType(MediaType.APPLICATION_JSON))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.data.id").value(("1250808601736515584")))
                .andExpect(jsonPath("$.data.name").value("Deluminator"));
        mockMvc.perform(MockMvcRequestBuilders.get("/artifacts/1250808601744904193").contentType(MediaType.APPLICATION_JSON))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.data.id").value(("1250808601744904193")))
